#ifndef _thomas_solver_h_included_ /// just to make sure nothing was defined erlier or somewhere else//*not neccessary here*//
#define _thomas_solver_h_included_
...
static inline void Copy3x3(const double Source[3][3],
			   double Target[3][3])
{
...
static inline void MultSca(double A[3][3], const double delta)
{
  A[0][0] = A[0][0] * delta; A[0][1] = A[0][1] * delta; A[0][2] = A[0][2] * delta;
  A[1][0] = A[1][0] * delta; A[1][1] = A[1][1] * delta; A[1][2] = A[1][2] * delta;
  A[2][0] = A[2][0] * delta; A[2][1] = A[2][1] * delta; A[2][2] = A[2][2] * delta;
}

static inline void Add3x3(const double A[3][3],
			  const double B[3][3],
			  const double Factor,
			  double C[3][3])
{
...
/*int main(void)
{
  ...
  SolveBlockTri(MAXSIZE, LHS, RHS, NRows);
